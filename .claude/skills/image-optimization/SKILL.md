...
description: Optimize how images are sourced, loaded, and rendered on sefa.is (next/image usage, remote hosts, hero/LCP images, alt text, OG image). Use when adding new images, touching the hero photo crossfade, or asked to improve image performance/SEO/accessibility.
---

# Image optimization

Sefa is a visually heavy marketing site (hero crossfade, product/salon
photography) where images are a major factor in both performance
...
and `twitter.images`; `public/og-image.jpg` exists and was generated by
screenshotting the real `Logo` + hero copy at exact size (see the
`logo-polish` skill for the regeneration process). If the wordmark, palette,
or hero copy changes meaningfully, regenerate it the same way rather than
letting it go stale or hand-authoring a disconnected graphic.
